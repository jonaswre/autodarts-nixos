...
.world3d{margin-top:14px;border:1px solid #ffffff1d;border-radius:12px;overflow:hidden;background:radial-gradient(circle at 50% 30%,#172430,#080c12 70%)}.world3d-head{display:flex;justify-content:space-between;gap:12px;padding:11px 14px;color:#aeb9c5;font-size:13px;align-items:center;flex-wrap:wrap}.world3d-head strong{color:#eef5f2}.world3d-meta{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.world3d-controls{display:flex;gap:5px;align-items:center}.world3d-controls button{border:1px solid #ffffff28;background:#17212b;color:#c9d5dd;border-radius:6px;padding:4px 9px;cursor:pointer}.world3d-controls button.active{background:#27e6a7;color:#07110e;border-color:#27e6a7}.camera-toggle{display:flex;align-items:center;gap:5px;cursor:pointer}.camera-toggle input{accent-color:#27e6a7}.world3d canvas{display:block;width:100%;height:460px;touch-action:none;cursor:grab}.world3d canvas:active{cursor:grabbing}.world3d-note{padding:0 14px 12px;color:#8e9dab;font-size:12px}
...
function world3DCard(l,scene){if(!scene?.darts?.length)return '';const worstFit=Math.max(...scene.darts.map(d=>finite(d.fit_residual_degrees)));return '<section class="world3d"><div class="world3d-head"><strong>Reconstructed 3D board world</strong><div class="world3d-meta"><span>'+scene.darts.length+' physical dart'+(scene.darts.length===1?'':'s')+' · worst fit '+worstFit.toFixed(2)+'°</span><div class="world3d-controls"><button type="button" data-world-view="perspective" class="active">3D</button><button type="button" data-world-view="front">Front</button><label class="camera-toggle"><input type="checkbox" data-camera-toggle> Cameras</label></div></div></div><canvas data-world3d="'+esc(l.sample_id)+'" role="img" aria-label="Interactive realistic 3D reconstruction of '+scene.darts.length+' physical darts"><p>3D preview requires canvas.</p></canvas><div class="world3d-note">Drag to orbit · wheel or pinch to zoom · directions are reconstructed; displayed dart length is '+finite(scene.darts[0].length_mm)+' mm.</div></section>'}
function mountWorld3D(canvas,scene){
  ...
  let yaw=-.58,pitch=.28,distance=760,showCameras=false;
  ...
  const updatePinch=()=>{if(pointers.size!==2){pinch=null;return}const points=[...pointers.values()],span=Math.hypot(points[0][0]-points[1][0],points[0][1]-points[1][1]);if(!pinch)pinch={span,distance};else{distance=Math.max(340,Math.min(1500,pinch.distance*pinch.span/span));draw()}};
  canvas.addEventListener('pointerdown',e=>{pointers.set(e.pointerId,[e.clientX,e.clientY]);drag=[e.clientX,e.clientY];canvas.setPointerCapture(e.pointerId);updatePinch()});canvas.addEventListener('pointermove',e=>{if(!pointers.has(e.pointerId))return;const next=[e.clientX,e.clientY];pointers.set(e.pointerId,next);if(pointers.size===1&&drag){yaw+=(next[0]-drag[0])*.008;pitch=Math.max(-1.3,Math.min(1.3,pitch+(next[1]-drag[1])*.008));drag=next;draw()}else updatePinch()});const release=e=>{pointers.delete(e.pointerId);drag=pointers.size===1?[...pointers.values()][0]:null;updatePinch()};canvas.addEventListener('pointerup',release);canvas.addEventListener('pointercancel',release);canvas.addEventListener('wheel',e=>{e.preventDefault();distance=Math.max(340,Math.min(1500,distance*Math.exp(e.deltaY*.001)));draw()},{passive:false});
  panel.querySelectorAll('[data-world-view]').forEach(button=>button.addEventListener('click',()=>{panel.querySelectorAll('[data-world-view]').forEach(value=>value.classList.toggle('active',value===button));if(button.dataset.worldView==='front'){yaw=0;pitch=0;distance=670}else{yaw=-.58;pitch=.28;distance=760}draw()}));panel.querySelector('[data-camera-toggle]').addEventListener('change',event=>{showCameras=event.target.checked;draw()});new ResizeObserver(draw).observe(canvas);draw()
}
