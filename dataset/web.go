...
async function sampleCard(s){const l=s.label,[setup,teacherLines]=await Promise.all([setupFor(l),teacherLinesFor(l)]);const before=(l.frames?.before||[]).map((f,i)=>frame(l,f,'before',i,setup,s.preview_darts?.before,[])).join('');const after=(l.frames?.after||[]).map((f,i)=>frame(l,f,'after',i,setup,s.preview_darts?.after,teacherLines)).join('');const orderedFrames=l.has_coordinates?after+before:before+after;return '<article class="sample"><div class="head"><div><div class="segment">'+title(l)+'</div>'+details(l)+'</div><div>'+new Date(l.captured_at).toLocaleString()+'<br>'+size(s.size_bytes)+' · '+(l.frame_sequence?.length||6)+' frames</div></div><div class="frames">'+orderedFrames+'</div></article>'}
