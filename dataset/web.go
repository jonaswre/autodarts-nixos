...
</style></head><body><main class="wrap"><div class="brand">auto<span>darts</span> dataset</div><p><a href="/api/export/webdataset.tar">Download training-ready WebDataset</a></p><section class="summary"><div class="metric">Training samples<strong id="count">–</strong></div><div class="metric">Excluded evidence<strong id="evidence">–</strong></div><div class="metric">Storage<strong id="used">–</strong><div class="bar"><i id="bar"></i></div></div><div class="metric">Quota<strong id="quota">–</strong></div></section><div class="toolbar"><span>Showing the latest 24 training samples</span><div class="overlay-controls"><div class="legend"><span><i class="roi-key"></i>ROI</span><span><i class="double-key"></i>Double</span><span><i class="treble-key"></i>Treble</span><span><i class="bull-key"></i>Bull</span><span><i class="calibration-key"></i>Calibration</span><span><i class="dart-key"></i>Teacher dart</span><span><i class="line-key"></i>Tip-flight line</span></div><label class="toggle"><input id="featuresToggle" type="checkbox" checked>Detected board features</label></div></div><section id="samples" class="samples"><div class="empty">Loading samples…</div></section></main>
<script>
...
async function sampleCard(s){const l=s.label,[setup,teacherLines]=await Promise.all([setupFor(l),teacherLinesFor(l)]);return '<article class="sample"><div class="head"><div><div class="segment">'+title(l)+'</div>'+details(l)+'</div><div>'+new Date(l.captured_at).toLocaleString()+'<br>'+size(s.size_bytes)+' · '+(l.frame_sequence?.length||6)+' frames</div></div><div class="frames">'+(l.frames?.before||[]).map((f,i)=>frame(l,f,'before',i,setup,s.preview_darts?.before,[])).join('')+(l.frames?.after||[]).map((f,i)=>frame(l,f,'after',i,setup,s.preview_darts?.after,teacherLines)).join('')+'</div></article>'}
