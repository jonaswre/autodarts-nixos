...
.world3d-summary{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:14px;padding:12px 14px;border:1px solid #ffffff1d;border-radius:10px;background:#0b1118;color:#aeb9c5}.world3d-summary strong{display:block;color:#eef5f2;margin-bottom:3px}.open-world3d,.world3d-controls button,.world3d-close{border:1px solid #ffffff28;background:#17212b;color:#c9d5dd;border-radius:7px;padding:7px 11px;cursor:pointer}.open-world3d{background:#12372f;border-color:#27e6a766;color:#9ff5d7;font-weight:700;white-space:nowrap}.open-world3d:hover,.open-world3d:focus-visible{background:#185044}.world3d-dialog{width:min(1200px,96vw);max-width:none;padding:0;border:1px solid #ffffff2b;border-radius:14px;background:#080d13;color:#eef5f2;box-shadow:0 28px 100px #000d}.world3d-dialog::backdrop{background:#020509d9;backdrop-filter:blur(4px)}.world3d{overflow:hidden;background:radial-gradient(circle at 50% 30%,#172430,#080c12 70%)}.world3d-head{display:flex;justify-content:space-between;gap:12px;padding:13px 15px;color:#aeb9c5;font-size:13px;align-items:center;flex-wrap:wrap;border-bottom:1px solid #ffffff17}.world3d-head strong{display:block;color:#eef5f2;font-size:16px}.world3d-meta{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.world3d-controls{display:flex;gap:5px;align-items:center}.world3d-controls button.active{background:#27e6a7;color:#07110e;border-color:#27e6a7}.world3d-close{font-size:19px;line-height:1;padding:5px 9px}.camera-toggle{display:flex;align-items:center;gap:5px;cursor:pointer}.camera-toggle input{accent-color:#27e6a7}.world3d canvas{display:block;width:100%;height:min(72vh,720px);min-height:460px;touch-action:none;cursor:grab}.world3d canvas:active{cursor:grabbing}.world3d-note{padding:10px 15px 13px;color:#8e9dab;font-size:12px;border-top:1px solid #ffffff12}
</style></head><body><main class="wrap"><div class="brand">auto<span>darts</span> dataset</div><p><a href="/api/export/webdataset.tar">Download training-ready WebDataset</a></p><section class="summary"><div class="metric">Training samples<strong id="count">–</strong></div><div class="metric">Excluded evidence<strong id="evidence">–</strong></div><div class="metric">Storage<strong id="used">–</strong><div class="bar"><i id="bar"></i></div></div><div class="metric">Quota<strong id="quota">–</strong></div></section><div class="toolbar"><span>Accepted throws first, then the latest world observations</span><div class="overlay-controls"><div class="legend"><span><i class="roi-key"></i>ROI</span><span><i class="double-key"></i>Double</span><span><i class="treble-key"></i>Treble</span><span><i class="bull-key"></i>Bull</span><span><i class="calibration-key"></i>Calibration</span><span><i class="dart-key"></i>Teacher dart</span><span><i class="line-key"></i>Tip-flight line</span></div><label class="toggle"><input id="featuresToggle" type="checkbox" checked>Detected board features</label></div></div><section id="samples" class="samples"><div class="empty">Loading samples…</div></section></main><dialog id="world3dDialog" class="world3d-dialog"><section class="world3d"><div class="world3d-head"><div><strong id="world3dTitle">Reconstructed 3D board world</strong><span id="world3dStats"></span></div><div class="world3d-meta"><div class="world3d-controls"><button type="button" data-world-view="perspective" class="active">3D</button><button type="button" data-world-view="front">Front</button><label class="camera-toggle"><input type="checkbox" data-camera-toggle> Cameras</label></div><button type="button" class="world3d-close" aria-label="Close 3D preview">×</button></div></div><canvas id="world3dCanvas" role="img" aria-label="Interactive realistic 3D dartboard reconstruction"><p>3D preview requires canvas.</p></canvas><div id="world3dNote" class="world3d-note"></div></section></dialog>
<script>
...
const world3dStorageKey='autodarts-dataset-3d-viewport-v1';
let world3dViewport={yaw:-.58,pitch:.28,distance:760,showCameras:false,view:'perspective'};
try{const stored=JSON.parse(localStorage.getItem(world3dStorageKey));if(stored&&['yaw','pitch','distance'].every(key=>Number.isFinite(Number(stored[key]))))world3dViewport={...world3dViewport,...stored}}catch(_){}
function rememberWorld3DViewport(value){world3dViewport={...value};try{localStorage.setItem(world3dStorageKey,JSON.stringify(world3dViewport))}catch(_){}}
function world3DCard(l,scene){if(!scene?.darts?.length)return '';const worstFit=Math.max(...scene.darts.map(d=>finite(d.fit_residual_degrees)));return '<section class="world3d-summary"><div><strong>Reconstructed 3D board world</strong><span>'+scene.darts.length+' physical dart'+(scene.darts.length===1?'':'s')+' · 3 calibrated cameras · worst fit '+worstFit.toFixed(2)+'°</span></div><button type="button" class="open-world3d" data-open-world3d="'+esc(l.sample_id)+'">Open 3D preview</button></section>'}
function mountWorld3D(canvas,scene){
  const ctx=canvas.getContext('2d');if(!ctx)return;
  const panel=canvas.closest('.world3d'),sectorOrder=[20,1,18,4,13,6,10,15,2,17,3,19,7,16,8,11,14,9,12,5],boardScale=scene.board_radius_mm/170;
  const dartColors=['#ff9f1c','#39c6ff','#d96cff','#75e66b'];
  let {yaw,pitch,distance,showCameras,view}=world3dViewport;
  const pointers=new Map();let drag=null,pinch=null;
  const projectFactory=(width,height)=>{const target=[0,0,30],position=[Math.sin(yaw)*Math.cos(pitch)*distance,Math.sin(pitch)*distance,Math.cos(yaw)*Math.cos(pitch)*distance],forward=vnorm(vsub(target,position)),right=vnorm(vcross(forward,[0,1,0])),up=vcross(right,forward),f=Math.min(width,height)*1.12;return p=>{const relative=vsub(p,position),depth=vdot(relative,forward);return [width/2+vdot(relative,right)*f/depth,height/2-vdot(relative,up)*f/depth,depth]}};
  const path=(project,points,close=false)=>{ctx.beginPath();points.forEach((p,i)=>{const q=project(p);i?ctx.lineTo(q[0],q[1]):ctx.moveTo(q[0],q[1])});if(close)ctx.closePath()};
  const polar=(radius,angle,z=0)=>[Math.sin(angle)*radius*boardScale,Math.cos(angle)*radius*boardScale,z];
  const circle=(radius,z=0,steps=100)=>Array.from({length:steps},(_,i)=>polar(radius,i/steps*Math.PI*2,z));
  const wedge=(inner,outer,start,end,z=.2,steps=5)=>{const points=[];for(let i=0;i<=steps;i++)points.push(polar(outer,start+(end-start)*i/steps,z));for(let i=steps;i>=0;i--)points.push(polar(inner,start+(end-start)*i/steps,z));return points};
  const stroke3D=(project,from,to,color,width,ratio)=>{const a=project(from),b=project(to),gradient=ctx.createLinearGradient(a[0],a[1],b[0],b[1]);gradient.addColorStop(0,'#f5f7f8');gradient.addColorStop(.28,color);gradient.addColorStop(.72,color);gradient.addColorStop(1,'#272d33');ctx.strokeStyle=gradient;ctx.lineWidth=width*ratio;ctx.lineCap='round';path(project,[from,to]);ctx.stroke()};
  const draw=()=>{const rect=canvas.getBoundingClientRect(),ratio=Math.min(2,devicePixelRatio||1),width=Math.max(1,Math.round(rect.width*ratio)),height=Math.max(1,Math.round(rect.height*ratio));if(canvas.width!==width||canvas.height!==height){canvas.width=width;canvas.height=height}ctx.clearRect(0,0,width,height);const project=projectFactory(width,height);ctx.lineJoin='round';ctx.lineCap='round';
    const glow=ctx.createRadialGradient(width*.5,height*.42,0,width*.5,height*.45,width*.55);glow.addColorStop(0,'#213343');glow.addColorStop(1,'#070b10');ctx.fillStyle=glow;ctx.fillRect(0,0,width,height);
    ctx.fillStyle='#15191c';path(project,circle(226,-26),true);ctx.fill();ctx.strokeStyle='#050607';ctx.lineWidth=28*ratio;ctx.stroke();ctx.fillStyle='#090b0d';path(project,circle(226,-2),true);ctx.fill();
    for(let sector=0;sector<20;sector++){const center=sector/20*Math.PI*2,start=center-Math.PI/20,end=center+Math.PI/20,dark=sector%2===0,sisal=dark?'#171819':'#e7d7b2',ring=dark?'#c51f3d':'#168c55';for(const [inner,outer,color] of [[15.9,99,sisal],[99,107,ring],[107,162,sisal],[162,170,ring]]){ctx.fillStyle=color;path(project,wedge(inner,outer,start,end),true);ctx.fill()}}
    ctx.fillStyle='#168c55';path(project,circle(15.9,.5),true);ctx.fill();ctx.fillStyle='#c51f3d';path(project,circle(6.35,1),true);ctx.fill();
    ctx.strokeStyle='#c5c9c9';ctx.lineWidth=1.35*ratio;for(let sector=0;sector<20;sector++){const boundary=sector/20*Math.PI*2-Math.PI/20;path(project,[polar(15.9,boundary,1),polar(170,boundary,1)]);ctx.stroke()}for(const radius of [6.35,15.9,99,107,162,170]){path(project,circle(radius,1));ctx.stroke()}
    ctx.fillStyle='#ede7d6';ctx.strokeStyle='#050607';ctx.lineWidth=3*ratio;ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='bold '+16*ratio+'px system-ui';for(let sector=0;sector<20;sector++){const q=project(polar(195,sector/20*Math.PI*2,2));ctx.strokeText(String(sectorOrder[sector]),q[0],q[1]);ctx.fillText(String(sectorOrder[sector]),q[0],q[1])}
    if(showCameras)for(const camera of scene.cameras||[]){const center=camera.center_mm,color=['#00d9ff','#b982ff','#ffe600'][camera.camera%3];ctx.strokeStyle=color+'55';ctx.lineWidth=1*ratio;ctx.setLineDash([7*ratio,7*ratio]);for(const dart of scene.darts){path(project,[center,dart.tip_mm]);ctx.stroke()}ctx.setLineDash([]);const q=project(center);ctx.fillStyle=color;ctx.beginPath();ctx.arc(q[0],q[1],6*ratio,0,Math.PI*2);ctx.fill();ctx.fillStyle='#dfe8ee';ctx.font=12*ratio+'px system-ui';ctx.textAlign='left';ctx.fillText('cam '+camera.camera,q[0]+9*ratio,q[1]-7*ratio)}
    for(const [index,dart] of scene.darts.entries()){const color=dartColors[index%dartColors.length],tip=dart.tip_mm,direction=vnorm(vsub(dart.flight_mm,tip)),reference=Math.abs(direction[1])<.9?[0,1,0]:[1,0,0],side=vnorm(vcross(direction,reference)),side2=vnorm(vcross(direction,side)),at=millimetres=>vadd(tip,vscale(direction,millimetres)),needleEnd=at(28),barrelEnd=at(75),shaftEnd=at(118),flightEnd=at(dart.length_mm),finBase=at(108);
      const shadow=project([tip[0]+7,tip[1]-7,-1]);ctx.fillStyle='#0008';ctx.beginPath();ctx.ellipse(shadow[0],shadow[1],13*ratio,7*ratio,0,0,Math.PI*2);ctx.fill();stroke3D(project,tip,needleEnd,'#aeb6bb',2.4,ratio);stroke3D(project,needleEnd,barrelEnd,'#aeb6bb',10,ratio);stroke3D(project,barrelEnd,shaftEnd,color,4.5,ratio);
      ctx.globalAlpha=.84;ctx.fillStyle=color;for(const axis of [side,side2])for(const sign of [-1,1]){path(project,[flightEnd,vadd(finBase,vscale(axis,22*sign)),shaftEnd],true);ctx.fill()}ctx.globalAlpha=1;const tipQ=project(tip);ctx.fillStyle='#f43f5e';ctx.beginPath();ctx.arc(tipQ[0],tipQ[1],4.5*ratio,0,Math.PI*2);ctx.fill();const labelQ=project(flightEnd),labelX=labelQ[0]+12*ratio,labelY=labelQ[1]+(index-(scene.darts.length-1)/2)*28*ratio;ctx.strokeStyle=color;ctx.lineWidth=1.5*ratio;ctx.beginPath();ctx.moveTo(labelQ[0],labelQ[1]);ctx.lineTo(labelX-4*ratio,labelY);ctx.stroke();ctx.font='bold '+14*ratio+'px system-ui';const labelWidth=ctx.measureText(dart.segment).width;ctx.fillStyle='#071017dd';ctx.fillRect(labelX-4*ratio,labelY-15*ratio,labelWidth+8*ratio,21*ratio);ctx.strokeStyle=color;ctx.strokeRect(labelX-4*ratio,labelY-15*ratio,labelWidth+8*ratio,21*ratio);ctx.fillStyle='#f7fafc';ctx.textAlign='left';ctx.fillText(dart.segment,labelX,labelY)}ctx.textAlign='left'};
  const remember=()=>rememberWorld3DViewport({yaw,pitch,distance,showCameras,view});
  const updatePinch=()=>{if(pointers.size!==2){pinch=null;return}const points=[...pointers.values()],span=Math.hypot(points[0][0]-points[1][0],points[0][1]-points[1][1]);if(!pinch)pinch={span,distance};else{distance=Math.max(340,Math.min(1500,pinch.distance*pinch.span/span));view='custom';remember();draw()}};
  canvas.addEventListener('pointerdown',e=>{pointers.set(e.pointerId,[e.clientX,e.clientY]);drag=[e.clientX,e.clientY];canvas.setPointerCapture(e.pointerId);updatePinch()});canvas.addEventListener('pointermove',e=>{if(!pointers.has(e.pointerId))return;const next=[e.clientX,e.clientY];pointers.set(e.pointerId,next);if(pointers.size===1&&drag){yaw+=(next[0]-drag[0])*.008;pitch=Math.max(-1.3,Math.min(1.3,pitch+(next[1]-drag[1])*.008));drag=next;view='custom';remember();draw()}else updatePinch()});const release=e=>{pointers.delete(e.pointerId);drag=pointers.size===1?[...pointers.values()][0]:null;updatePinch()};canvas.addEventListener('pointerup',release);canvas.addEventListener('pointercancel',release);canvas.addEventListener('wheel',e=>{e.preventDefault();distance=Math.max(340,Math.min(1500,distance*Math.exp(e.deltaY*.001)));remember();draw()},{passive:false});
  const cameraToggle=panel.querySelector('[data-camera-toggle]');cameraToggle.checked=showCameras;panel.querySelectorAll('[data-world-view]').forEach(button=>{button.classList.toggle('active',button.dataset.worldView===view);button.onclick=()=>{view=button.dataset.worldView;panel.querySelectorAll('[data-world-view]').forEach(value=>value.classList.toggle('active',value===button));if(view==='front'){yaw=0;pitch=0;distance=670}else{yaw=-.58;pitch=.28;distance=760}remember();draw()}});cameraToggle.onchange=event=>{showCameras=event.target.checked;remember();draw()};const observer=new ResizeObserver(draw);observer.observe(canvas);draw();return ()=>observer.disconnect()
}
let closeWorld3DRenderer=()=>{};
function mountWorldPreviews(entries){const scenes=new Map(entries.filter(s=>s.preview_3d).map(s=>[s.label.sample_id,s.preview_3d])),dialog=document.getElementById('world3dDialog');document.querySelectorAll('[data-open-world3d]').forEach(button=>button.onclick=()=>{const scene=scenes.get(button.dataset.openWorld3d);if(!scene)return;const worstFit=Math.max(...scene.darts.map(d=>finite(d.fit_residual_degrees)));dialog.querySelector('#world3dTitle').textContent='Reconstructed 3D board world';dialog.querySelector('#world3dStats').textContent=scene.darts.length+' physical dart'+(scene.darts.length===1?'':'s')+' · 3 calibrated cameras · worst fit '+worstFit.toFixed(2)+'°';dialog.querySelector('#world3dNote').textContent='Drag to orbit · wheel or pinch to zoom · directions are reconstructed; displayed dart length is '+finite(scene.darts[0].length_mm)+' mm.';closeWorld3DRenderer();const previous=document.getElementById('world3dCanvas'),canvas=previous.cloneNode(true);previous.replaceWith(canvas);dialog.showModal();closeWorld3DRenderer=mountWorld3D(canvas,scene)||(()=>{})});dialog.querySelector('.world3d-close').onclick=()=>dialog.close();dialog.onclick=event=>{if(event.target===dialog)dialog.close()};dialog.onclose=()=>{closeWorld3DRenderer();closeWorld3DRenderer=()=>{}}}
