{ lib, pkgs, ... }:
{
  imports = [ ./autodarts.nix ./kiosk.nix ];
  ...
  networking = {
    ...
    firewall.allowedTCPPorts = [ 22 3180 ];
  };
  ...
  boot = {
    ...
    consoleLogLevel = 0;
    kernelParams = [ "quiet" "loglevel=3" "udev.log_level=3" "i915.fastboot=1" ];
    loader = {
      timeout = 0;
  ...
  environment.systemPackages = with pkgs; [ curl git htop networkmanager ];
